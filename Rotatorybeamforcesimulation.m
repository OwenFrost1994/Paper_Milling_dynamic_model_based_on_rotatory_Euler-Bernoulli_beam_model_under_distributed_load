...
Fxs=0;
Fys=0;
F=zeros(2,Ns*Cn);%%存储两个方向的有阻尼梁切削力
Fd=zeros(2,Ns*Cn);%%存储两个方向的无阻尼梁切削力
...
K1=(M/(Dt^2)+KS);
K1=inv(K1);
%%基于无阻尼梁模型计算切削力同步用有限差分法计算梁的振动位移和速度%%
for i=1:1:Ns*Cn;
    Ca=Ca+DC;%%微元角度叠加计算刀具的转动角
    if Ca>=2*pi%%考虑刀具多个旋转周期，累加的刀具角度超过一周就减去一个2π
        Ca=Ca-2*pi;
...
subplot(2,1,1)
% plot(Fm(n3:n4,1),F(1,n1:n2),'m-','Markersize',7,'Markerface','white','linewidth',2.0);%%这里要注意F是3×N,读取的数据是N×4
% hold on;
...
set(get(gca,'XLabel'),'Fontsize',20)
set(get(gca,'YLabel'),'Fontsize',20)
% legend('Rotatory Beam','Static Beam','Traditonal','Measured')
legend('Static Beam','Traditonal','Measured')

subplot(2,1,2)
% plot(Fm(n3:n4,1),F(2,n1:n2),'m-','Markersize',7,'Markerface','white','linewidth',3.0);
% hold on;
plot(Fm(n3:n4,1),Fd(1,n1:n2),'r-','Markersize',7,'Markerface','white','linewidth',1.0);
hold on;
plot(Fm(n3:n4,1),Fs(2,n1:n2),'b-.','Markersize',7,'Markerface','white','linewidth',2.0);
hold on;
